...
def subsample_unet(unet: UNet2DConditionModel, scale_factors: Union(dict, float), metric="L1") -> UNet2DConditionModel:
    ...
    #         )
    def fn_recursive_subsample_layer(module: torch.nn.Module):
        if isinstance(module, nn.Linear) or isinstance(module, nn.Conv2d) or isinstance(module, nn.MaxPool2d) or isinstance(nn.AvgPool2d) or isinstance(module, nn.GroupNorm) or isinstance(module, nn.SiLU):
            pruned_state_dict={}
            pruned = prune_layer(layer=module, prune_ratio=scale_factors[module._get_name], metric=metric)
            #are we creating an entirely new state dict here?
            pruned_state_dict[module._get_name] = pruned
        elif isinstance(module, ResnetBlock2D): 
            state_dict={}
            for child in module.children():
                state_dict.update(fn_recursive_subsample_layer(child))
            
        elif isinstance(module, Transformer2DModel):
            for child in module.children():
                fn_recursive_subsample_layer(child)
...
def prune_layer(layer, prune_ratio=0.0, target="weight", strategy="structured", metric="L1"):
    ...
    if strategy == "structured":
        if metric == "L1":
            prune.ln_structured(layer, name=target, amount=prune_ratio, n=1, dim=0)
        if metric == "L2":
            prune.ln_structured(layer, name=target, amount=prune_ratio, n=2, dim=0)
        if metric == "random":
            prune.random_structured(layer, name=target, amount=prune_ratio)
    
    pruned_weight = weight[layer.weight_mask.sum(dim=(1, 2, 3)) != 0]
    pruned_bias = layer.bias.data[layer.weight_mask.sum(dim=(1, 2, 3)) != 0]

    #returns a pruned state dict for this particular layer
    return {"weight":pruned_weight, "bias":pruned_bias}